...
export function EditProjectPage() {
  ...
  function toggleExistingImageRemoval(publicId) {
    if (!publicId) return;
  ...
  function validate() {
    ...
    const keptExistingImageCount = existingImages.length - removedImagePublicIds.length;
    const finalImageCount = keptExistingImageCount + newImages.length;

    if (finalImageCount > 1) errs.images = `Too many images — maximum 1 allowed.`;
    return errs;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (removedImagePublicIds.length > 0) {
        fd.append('removeImagePublicIds', JSON.stringify(removedImagePublicIds));
      }
  ...
  return (
    <div className="admin-page">
      ...
      <form id="edit-project-form" className="admin-card admin-form-wide" onSubmit={handleSubmit} noValidate>
        ...
        {existingImages.length > 0 && (
          <div className="admin-form-field">
            ...
            <p className="admin-form-field__hint">
              The existing image will be kept unless you remove it.
            </p>
            <div className="admin-existing-images">
              {existingImages.map((img, i) => {
                const isMarkedForRemoval = removedImagePublicIds.includes(img.publicId);

                return (
                  <div
                    key={img.publicId || i}
                    className={`admin-existing-images__item${isMarkedForRemoval ? ' admin-existing-images__item--removed' : ''}`}
                  >
                    <img
                      src={img.url}
                    ...
                    <button
                      type="button"
                      className={`admin-existing-images__action${isMarkedForRemoval ? ' admin-existing-images__action--undo' : ''}`}
        ...
        {/* New image – only show upload if no existing image is kept */}
        {(existingImages.length - removedImagePublicIds.length) === 0 && (
          <FileUpload
            id="ep-images"
